#!/usr/bin/env bash
# Evaluate all 5 models on the configured datasets.
# Writes results/*.json files and prints a summary table.
set -euo pipefail
...
# Datasets we keep in this repo's final eval suite.
DATASETS=(
...
# Models: base HF model + local checkpoints (baselines + phase2).
MODELS=(
  "Qwen/Qwen2.5-0.5B"
...
for m_idx in "${!MODELS[@]}"; do
  ...
  for d_idx in "${!DATASETS[@]}"; do
    ...
    log_file="logs/eval_${model_name}_${dataset_name}.log"
...
echo "Model                  | GSM8K  | MATH   | SVAMP  | MultiA"
echo "-----------------------|--------|--------|--------|--------"
